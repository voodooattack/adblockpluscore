...
Filter.prototype =
{
  ...
  *subscriptions()
  ...
  {
    buffer.push("[Filter]");
    buffer.push("text=" + this.toString());
  },

  toString()
  {
    return decompressText(this.text);
  }
};
...
{
  let hash = crc32(text);
  let filter = Filter.knownFilters.get(hash);
  if (filter)
    return filter;
  ...
  Filter.knownFilters.set(hash, filter);
  return filter;
};
